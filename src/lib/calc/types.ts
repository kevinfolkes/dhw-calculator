...
export interface CalcResult {
  ...
  /** Recovered heat capacity (BTU/hr while running) for `central_chp`.
   *  = `chpElectricKW × chpHeatToPowerRatio × 3412`. Zero for all other
   *  system types so tabs can detect the CHP case via
   *  `chpHeatRecoveryBTUH > 0`. */
  chpHeatRecoveryBTUH: number;
  /** Total recoverable BTU available per year for `central_chp` BEFORE
   *  the utilization factor and DHW-load cap apply
   *  (= `chpHeatRecoveryBTUH × chpAnnualRunHours`). Zero for all other
   *  system types. */
  chpAnnualRecoveryBTU: number;
  /** Annual BTU contributed by recovered CHP heat to DHW for `central_chp`
   *  (capped by annual DHW load AND by recoverable BTU × utilization
   *  factor). Zero for all other system types. */
  chpAnnualContributionBTU: number;
  /** Fraction of annual DHW load covered by recovered CHP heat for
   *  `central_chp` (0–1). Zero for all other system types. */
  chpCoverageFraction: number;
  /** Annual electricity generated by the CHP unit (kWh) for `central_chp`
   *  (= `chpElectricKW × chpAnnualRunHours`). Informational only — this
   *  electricity is consumed by the building or net-metered; it is NOT
   *  attributed to the DHW account. Zero for all other system types. */
  chpAnnualElectricGeneratedKWh: number;
  /** Echo of the user-selected HPWH source-coupling mode. Always populated
   *  (defaults to "air_mech_room"); resolved + sentinel companion field
   *  `hpwhEffectiveSourceTempF` reflects whether the override actually
   *  applied to the active system. */
  hpwhSourceMode: HpwhSourceMode;
  /** Resolved source temperature (°F) the HPWH compressor sees in this
   *  scenario. Equals the configured `effectiveHpwhAmbient` for systems
   *  not eligible for the ground-loop override; equals `GROUND_LOOP_TEMP_F`
   *  when `hpwhSourceMode === "ground_loop"` AND the active system is
   *  central_hpwh / central_hybrid / central_per_floor. Zero for systems
   *  with no HPWH (gas / steam / resistance) so tabs can detect "is the
   *  HPWH source override active?" via `hpwhEffectiveSourceTempF > 0`
   *  with `hpwhSourceMode === "ground_loop"`. */
  hpwhEffectiveSourceTempF: number;
